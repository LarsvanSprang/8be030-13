...
def intensity_based_registration_affine_mi(s1, s2):
    ...
    # initial values for the parameters (identity transformation)
    x = np.array([0., 1., 1., 0., 0., 0., 0.])

    # similarity function using mutual information
...
def intensity_based_registration_affine_mi_th(I, Im, iter):
    x = np.array([0., 1., 1., 0., 0., 0., 0.])
    fun = lambda x: reg.affine_mi(I, Im, x, return_transform=False)
