...
def arbitrary_rotation():
    ...
    T1 = util.t2h(np.eye(2), -Xt)

    # Step 2: Rotate 45 degrees
    theta = np.radians(45)
    R = np.array([[np.cos(theta), -np.sin(theta)],
                  [np.sin(theta),  np.cos(theta)]])
    T2 = util.t2h(R, np.zeros(2))

    # Step 3: Translate back
    T3 = util.t2h(np.eye(2), Xt)
...
def ls_solve_test():
    from registration import ls_solve
    #------------------------------------------------------------------#
...
def correlation_test():
    ...
    assert abs(C2 - 1) < 1e-10, "Linear intensity transformation test failed"
    #------------------------------------------------------------------#
...
def ngradient_test():
    ...
    square = lambda x: x**2
    x2 = np.array([3.0])
    g2 = reg.ngradient(square, x2)
    assert abs(g2 - 2 * x2) < 1e-5, "Numerical gradient failed (square test)"

    # Test 3: f(x) = sin(x), derivative is cos(x)
    sine = lambda x: np.sin(x)
    x3 = np.array([np.pi / 3])
    g3 = reg.ngradient(sine, x3)
    assert abs(g3 - np.cos(x3)) < 1e-5, "Numerical gradient failed (sine test)"
    #------------------------------------------------------------------#
