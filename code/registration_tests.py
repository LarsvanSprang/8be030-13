...
def combining_transforms():
    ...
    X_t1 = reg.reflect(-1,1).dot(reg.rotate(np.pi/2)).dot(X)
    X_t2 = reg.reflect(-1,1).dot(reg.rotate(np.pi/3)).dot(X)
    X_t3 = reg.reflect(-1,1).dot(reg.shear(1.2,3)).dot(X)
    
    fig = plt.figure(figsize=(12,5))
    
    ax1 = fig.add_subplot(241, xlim=(-4,4), ylim=(-4,4))
    ax2 = fig.add_subplot(242, xlim=(-4,4), ylim=(-4,4))
    ax3 = fig.add_subplot(243, xlim=(-4,4), ylim=(-4,4))
    
    util.plot_object(ax1, X_t1)
    util.plot_object(ax2, X_t2)
    util.plot_object(ax3, X_t3)
...
def arbitrary_rotation():
    ...
    t = X[:,0] 
    phi = np.pi/4
    T = util.t2h(reg.rotate(phi),t)
    T = T.dot(util.t2h(reg.identity(),-t))
...
def mutual_information_test():
    ...
    img1 = np.random.randint(0, 256, size=(512, 512), dtype=np.uint8)
    img2 = np.random.randint(0, 256, size=(512, 512), dtype=np.uint8)

    p2 = reg.joint_histogram(img1,img2)
    mi = reg.mutual_information(p2)
